...
function initTocScrollSpy() {
  ...
  function updateToc() {
    ...
    tocLinks.forEach(link => link.classList.remove('font-bold', 'text-primary-500', 'dark:text-primary-300'));
    
    const currentId = headingPositions[currentHeadingIndex]?.id;
    if (currentId && idToTocLink[currentId]) {
      idToTocLink[currentId].classList.add('font-bold', 'text-primary-500', 'dark:text-primary-300');
    }
  }
